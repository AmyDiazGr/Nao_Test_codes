...
class SonarHandlerModule(ALModule):
    ...
    def move_laterally(self, direction):
        ...
        if direction == 'left':
            self.motion.moveTo(0, 0, 0.5)  # Aumentar el giro
        elif direction == 'right':
            self.motion.moveTo(0, 0, -0.5)  # Aumentar el giro en dirección opuesta
            
        self.motion.waitUntilMoveIsFinished()  # Esperar hasta que el giro se complete
        self.is_turning = True
        time.sleep(self.move_delay)  # Pequeña pausa después del giro
        self.is_turning = False
